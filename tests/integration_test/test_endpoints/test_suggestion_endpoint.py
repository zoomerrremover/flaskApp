...
def test_get_suggestions_by_user_successful(
        client,
...
def test_get_suggestions_by_non_existent_user(client):
    ...
    params = urlencode({"id": 999999, "limit": 10})
    response = client.get(f"/suggestion/by_user?{params}")
    assert response.status_code == HTTPStatus.NO_CONTENT


def test_get_suggestions_by_article_successful(
        client,
        registered_suggestion,
        suggestion_data,
        app
...
def test_create_suggestion_successful_user_role(
        client,
        registered_article,
        suggestion_data
):
...
def test_create_suggestion_successful_editor_role(
        client,
        registered_article,
        suggestion_data
):
...
def test_create_suggestion_unauthorized(
        client,
        registered_article,
        suggestion_data
):
...
def test_create_suggestion_missing_title(
        client,
        registered_article,
        suggestion_data
):
...
def test_create_suggestion_invalid_data_title_too_long(
        client,
        registered_article,
        suggestion_data
):
...
def test_create_suggestion_invalid_data_text_content_too_long(
        client,
        registered_article,
        suggestion_data
):
    ...
    headers = {"Authorization": f"Bearer {access_token}"}
    payload = next(suggestion_data(article_id=article_info["id"]))
    payload["text_content"] = "a" * 8001
    response = client.post("/suggestion/", json=payload, headers=headers)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_create_suggestion_invalid_article_id(
        client,
        registered_user,
        suggestion_data
):
    ...
    access_token = user_data["access_token"]
    headers = {"Authorization": f"Bearer {access_token}"}
    payload = next(suggestion_data(article_id=999999))
    response = client.post("/suggestion/", json=payload, headers=headers)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_star_suggestion_successful(
        client,
        registered_suggestion
):
...
def test_star_suggestion_unauthorized(client, registered_suggestion):
    ...
    suggestion_info, _, _, _ = next(registered_suggestion())
    params = urlencode({"id": suggestion_info["id"]})
    response = client.post(f"/suggestion/star?{params}")
    assert response.status_code == HTTPStatus.FORBIDDEN
...
def test_unstar_suggestion_successful(
        client,
        registered_suggestion,
        app
):
...
def test_unstar_suggestion_unauthorized(client, registered_suggestion):
    ...
    suggestion_info, _, _, _ = next(registered_suggestion())
    params = urlencode({"id": suggestion_info["id"]})
    response = client.post(f"/suggestion/unstar?{params}")
    assert response.status_code == HTTPStatus.FORBIDDEN
...
def test_update_suggestion_successful_by_owner(
        client,
        registered_suggestion,
        suggestion_data
):
    ...
    (
            ...
            _,
            _
    ) = next(registered_suggestion(user_role="user"))
    access_token = user_data["access_token"]
    headers = {"Authorization": f"Bearer {access_token}"}
    new_data = next(suggestion_data(article_id=suggestion_info["article_id"]))
    new_data["id"] = suggestion_info["id"]
    response = client.patch(
        f"/suggestion/?{urlencode(new_data)}",
        headers=headers
    )
    assert response.status_code == HTTPStatus.OK
    assert response.data == b""
    get_response = client.get(
        f"/suggestion/?{urlencode({'id': suggestion_info['id']})}"
    )
    assert get_response.status_code == HTTPStatus.OK
...
def test_update_suggestion_successful_by_editor(
        client,
        registered_suggestion,
        suggestion_data
):
    ...
    (
            ...
            _,
            _
    ) = next(registered_suggestion(user_role=UserRolesEnum.editor.value))
    access_token = editor_data["access_token"]
    headers = {"Authorization": f"Bearer {access_token}"}
    new_data = next(suggestion_data(article_id=suggestion_info["article_id"]))
    new_data["id"] = suggestion_info["id"]
...
def test_update_suggestion_invalid_data_title_too_long(client, registered_suggestion, suggestion_data):
    ...
    update_params = {"id": suggestion_info["id"], "title": "a" * 33}
    response = client.patch(f"/suggestion/?{urlencode(update_params)}", headers=headers)
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_delete_suggestion_unauthorized_no_token(client, registered_suggestion):
    ...
    suggestion_info, _, _, _ = next(registered_suggestion())
    delete_params = urlencode({"id": suggestion_info["id"]})
    response = client.delete(f"/suggestion/?{delete_params}")
    assert response.status_code == HTTPStatus.FORBIDDEN
...
def test_delete_suggestion_not_found(client, registered_user):
    ...
    delete_params = urlencode({"id": 999999})
    response = client.delete(f"/suggestion/?{delete_params}", headers=headers)
    assert response.status_code == HTTPStatus.UNAUTHORIZED

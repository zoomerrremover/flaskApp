...
def test_registration_missing_username(client, user_data):
...
def test_registration_missing_password(client, user_data):
    payload = user_data.copy()
...
def test_registration_missing_email(client, user_data):
    payload = user_data.copy()
    del payload["email"]
    response = client.post("/registration/", json=payload)
    assert response.status_code == 400
...
def test_registration_invalid_email_format(client, user_data):
    payload = user_data.copy()
...
def test_registration_invalid_password_format(client, user_data):
    payload = user_data.copy()
...
def test_double_registration(client, user_data):
...
def test_registration_with_extra_fields(client, user_data):
    payload = user_data.copy()
...
def test_registration_username_with_spaces(client, user_data):
    payload = user_data.copy()
...
def test_registration_email_with_spaces(client, user_data):
    payload = user_data.copy()

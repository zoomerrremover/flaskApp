...
def test_suggestion_get_model_valid_data(base_suggestion_get_data):
    ...
    assert suggestion_get.likes == 5
    assert suggestion_get.dislikes == 1
...
def test_suggestion_get_model_likes_not_int(base_suggestion_get_data):
    ...
    """
    invalid_data = base_suggestion_get_data.copy()
    invalid_data["likes"] = "five"
    with pytest.raises(ValidationError) as exc_info:
        SuggestionGetModel(**invalid_data)
    assert "likes" in str(exc_info.value)


def test_suggestion_get_model_dislikes_not_int(base_suggestion_get_data):
    """
    Tests that SuggestionGetModel raises ValidationError if dislikes is not an int.
    """
    invalid_data = base_suggestion_get_data.copy()
    invalid_data["dislikes"] = 1.5
    with pytest.raises(ValidationError) as exc_info:
        SuggestionGetModel(**invalid_data)
    assert "dislikes" in str(exc_info.value)

...
from app.db.service.user_service import (get_users, update_user_by_id,
...
def route_get_users(model: UserGet):
    user_id = model.id
    ...
    if user_id:
        ...
        users = get_users()
    return [jsonify(data) for data in users ]


@user_route.route("/", methods=['PATCH'])
@require_auth()
@validate_model_request(UserUpdate)
def route_update_user_password(data: UserUpdate):
...
def route_delete_user(data: UserDelete):
    user_id = data.id
